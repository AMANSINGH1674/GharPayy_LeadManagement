...
import { Plus, Trash2, UserCog, Building2, User, Save, LogOut } from 'lucide-react';
      ...
      >
        <Tabs defaultValue="team" className="space-y-6">
          ...
          <TabsContent value="profile">
            <ProfileTab user={user} signOut={signOut} onSignedOut={() => navigate('/auth')} />
          </TabsContent>
        </Tabs>
      </motion.div>
    </AppLayout>
...
function ProfileTab({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (name && user?.id) {
        ...
        if (profileError) console.warn('Profile update:', profileError);
      }
  ...
  return (
    <div className="space-y-6 max-w-md">
      ...
      <div className="kpi-card border-border">
        <h3 className="font-display font-semibold text-xs mb-2">Session</h3>
        <p className="text-[10px] text-muted-foreground mb-4">
          Sign out of this device. You will need to sign in again to access the dashboard.
        </p>
        <Button
          variant="outline"
          size="sm"
          ...
          disabled={loggingOut}
        >
          <LogOut size={12} /> {loggingOut ? 'Signing out...' : 'Sign out'}
        </Button>
      </div>
    </div>
  );
