...
const Dashboard = () => {
  ...
  // Stuck chats: leads in non-terminal stages with no activity for 3+ days
  const stuckCutoff = (() => {
    const d = new Date();
  ...
  const stuckLeads =
    leads?.filter(l => {
      ...
      const last = new Date(l.last_activity_at || l.updated_at || l.created_at);
  ...
  return (
    <AppLayout title="Dashboard" subtitle="Real-time overview of your sales pipeline">
      ...
      {/* KPIs */}
      <motion.div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6" variants={container} initial="hidden" animate="show">
      ...
      <motion.div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8" variants={container} initial="hidden" animate="show">
        ...
        <KpiCard title="Stuck Chats" value={stuckLeads.length} icon={<AlertTriangle size={17} />} color="hsl(var(--warning))" />
      </motion.div>
          ...
          <div className="space-y-2">
            {(reminders || []).slice(0, 5).map(r => (
              <div key={r.id} className={`flex items-center justify-between p-3 rounded-xl ${isPast(new Date(r.reminder_date)) ? 'bg-destructive/5 border border-destructive/15' : 'bg-secondary/50'}`}>
                <div>
                  <p className="text-xs font-medium text-foreground">{r.leads?.name}</p>
