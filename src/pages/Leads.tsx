...
import { useLeadsPaginated, useUpdateLead, useAgents, useCreateVisit, type LeadWithRelations } from '@/hooks/useCrmData';
import { useBulkUpdateLeads, useDeleteLeads } from '@/hooks/useLeadDetails';
import { useCreateBooking } from '@/hooks/useBookings';
import { PIPELINE_STAGES, SOURCE_LABELS, type PipelineStage } from '@/types/crm';
...
const Leads = () => {
  ...
  const handleBulkStatus = async (status: string) => {
    if (selectedIds.size === 0) return;
    try {
      await bulkUpdate.mutateAsync({ ids: Array.from(selectedIds), updates: { status: status as PipelineStage } });
        ...
        for (const lead of selectedLeads) {
          if (lead.property_id) {
            try {
              ...
              console.error('Failed to auto-create visit for lead', lead.id, vErr);
            }
          }
        }
        if (createdCount > 0) {
          toast.success(`${createdCount} visits auto-scheduled for tomorrow`);
        }
        if (createdCount < selectedIds.size) {
          toast.warning(`${selectedIds.size - createdCount} leads had no property assigned and were skipped for visit creation`);
        }
      }
  ...
  const handleInlineStatus = async (lead: LeadWithRelations, newStatus: string) => {
    try {
      await updateLead.mutateAsync({ id: lead.id, status: newStatus as PipelineStage });
      
      // When marking as booked from the table, ensure a basic booking exists
      if (newStatus === 'booked') {
      ...
      // When marking as visit scheduled, auto-create a visit record if property is known
      if (newStatus === 'visit_scheduled') {
        if (lead.property_id) {
          ...
          toast.warning('Lead status updated, but please assign a property to auto-create a visit record.', {
            duration: 5000,
          });
        }
      }
