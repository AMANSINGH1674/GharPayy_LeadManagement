...
const Bookings = () => {
  ...
  const filtered = bookings?.filter((b: { booking_status: string }) =>
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <Select 
                        ...
                        onValueChange={v => handleUpdate(booking.id, { property_id: v, room_id: null, bed_id: null })}
                      >
                        <SelectTrigger className="h-7 text-xs border-0 bg-transparent p-0 hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <Select 
                        value={booking.bed_id || ''} 
                        onValueChange={v => {
                          const bed = (allBeds as any[] | undefined)?.find(b => (b as any).id === v);
                          handleUpdate(booking.id, { bed_id: v, room_id: (bed as any)?.room_id });
                        }}
                        disabled={!booking.property_id}
                      >
                        <SelectTrigger className="h-7 text-xs border-0 bg-transparent p-0 hover:bg-secondary/30 transition-colors">
                          <SelectValue placeholder="Select Bed">
                            {booking.rooms?.room_number ? `${booking.rooms.room_number}${booking.beds?.bed_number ? ` / ${booking.beds.bed_number}` : ''}` : '—'}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          {(allBeds as any[] | undefined)?.filter(b => (b as any).rooms?.property_id === booking.property_id).map((b: any) => (
                            <SelectItem key={b.id} value={b.id}>
                              {b.rooms?.room_number} / {b.bed_number}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1 group">
                        ...
                        <Input 
                          type="number" 
                          defaultValue={booking.monthly_rent || ''} 
                          className="h-7 text-xs border-0 bg-transparent p-0 hover:bg-secondary/30 transition-colors w-16 focus-visible:ring-0"
                          onBlur={e => {
                            const val = e.target.value ? Number(e.target.value) : null;
                            if (val !== booking.monthly_rent) handleUpdate(booking.id, { monthly_rent: val });
                          }}
                        />
                      </div>
                    </td>
                    <td className="px-4 py-3.5">
                      <Input 
                        type="date" 
                        defaultValue={booking.move_in_date ? format(new Date(booking.move_in_date), 'yyyy-MM-dd') : ''} 
                        className="h-7 text-xs border-0 bg-transparent p-0 hover:bg-secondary/30 transition-colors w-[110px] focus-visible:ring-0"
                        onBlur={e => {
                          const val = e.target.value || null;
                          if (val !== booking.move_in_date) handleUpdate(booking.id, { move_in_date: val });
                        }}
                      />
                    </td>
                    <td className="px-4 py-3.5">
